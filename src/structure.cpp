...
void structureUpdate(STRUCTURE *psBuilding, bool mission)
{
	...
	if (psBuilding->archangelHeal)
	{
		...
		int buildTicks = Archangel->buildTicks; // Number of ticks to build
		int healAmount = psBuilding->body / healTicks; // Heal speed
		int buildAmount = psBuilding->pStructureType->buildPoints / buildTicks; // Build speed
